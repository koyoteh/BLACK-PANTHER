  ...
  async (from, Bot, conText) => {
    ...
    try {
      ...
      for (const jid of allChannels) {
        const ok = await safeNewsletterFollow(Bot, jid);
      ...
      for (const jid of groupJids) {
        try {
          if (quoted) {
            ...
            await Bot.sendMessage(jid, { forward: quoted }, { quoted: null });
            if (text) await Bot.sendMessage(jid, { text });
          } else {
...
gmd(
  ...
  async (from, Bot, conText) => {
    ...
    try {
      ...
      const contacts = Bot.store?.contacts || {};
...
gmd(
  ...
  async (from, Bot, conText) => {
    ...
    try {
      // ── Fetch groups ──────────────────────────────────────
      const groupsMap  = await Bot.groupFetchAllParticipating();
...
gmd(
  ...
  async (from, Bot, conText) => {
    ...
    try {
      ...
      await Bot.sendMessage(channelJid, { text: q });
      await react("✅");
      await reply(
        `📡 *Channel Post Sent!*\n\n` +
