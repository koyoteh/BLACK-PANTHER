...
addCmd({
    ...
    handler: async (ctx) => {
        ...
        menu += `╭━━━⬣ *${now('DD MMM YYYY')}* ⬣━━━╮\n`;
        menu += `┃ ⏰ *Time:* ${now('hh:mm:ss A')}\n`;
        menu += `╰━━━━━━━━⬣\n\n`;
        menu += `╭━━━⬣ *CATEGORIES* ⬣━━━╮\n`;
        ...
        await sendButtons(ctx.sock, ctx.from, {
            ...
            text:   menu,
            footer: `© ${config.BOT_NAME} ${new Date().getFullYear()}`,
    ...
    handler: async (ctx) => {
        ...
        for (const cat of CATS) {
            ...
            cmds.forEach((c) => {
                ...
                body += '\n';
                if (c.desc)  body += `┃    ↳ _${c.desc}_\n`;
                if (c.usage) body += `┃    📌 \`${p}${c.usage}\`\n`;
                total++;
            });
            body += `╰━━━━━━━━⬣\n\n`;
        }
