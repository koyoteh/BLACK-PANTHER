...
export default {
    name: 'generalmenu',
    aliases: ['genmenu', 'generalcmds'],
    description: 'Displays the General commands menu',
    run: async (context) => {
        ...
        let commandFiles = [];
        try {
            commandFiles = fs.readdirSync(PLUGIN_DIR)
                .filter(f => f.endsWith('.js') && !f.endsWith('menu.js') && f !== 'menu.js');
        } catch {}
